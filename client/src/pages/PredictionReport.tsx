...
export default function PredictionReport() {
  ...
  const clinicalInterpretation =
    (prediction as any).clinical_interpretation ||
    (prediction as any).clinicalInterpretation ||
    "";
  const recommendations = (prediction as any).recommendations || {};
  const modelPerformance =
    (prediction as any).model_performance ||
    (prediction as any).modelPerformance ||
    {};
  const nextSteps =
    (prediction as any).next_steps || (prediction as any).nextSteps || [];
  const biasAnalysis =
    prediction.biasAnalysis || (prediction as any).bias_analysis || {};
  ...
  // Check if we have SHAP values or should fall back to importance
  const hasShapValues = featureData.some((f) => f.shap_value !== 0);
  const chartDataKey = hasShapValues ? "shap_value" : "importance";
  const chartTitle = hasShapValues
    ? "Clinical Feature Attribution (SHAP Values)"
    : "Feature Importance";

  const handleSimulate = async () => {
    if (!id || !prediction.topFeatures || prediction.topFeatures.length === 0)
      return;
    try {
    ...
    <Layout>
      <header className="mb-8 flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-slate-500 font-medium uppercase tracking-widest">
            Model Fidelity: {(prediction.confidence * 100).toFixed(1)}%
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col items-center">
          ...
          <div className="mt-4 w-full p-3 bg-slate-50 rounded-xl border border-slate-100">
            <p className="text-[10px] text-slate-400 uppercase font-bold mb-1 tracking-widest">
              Uncertainty Metric (MC Dropout)
            </p>
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                <div
                  className="h-full bg-slate-400"
                  style={{ width: `${(prediction.uncertainty || 0.1) * 100}%` }}
                />
              </div>
              <span className="text-xs font-mono text-slate-600">
                ±{((prediction.uncertainty || 0.1) * 100).toFixed(1)}%
              </span>
            </div>
            <p className="text-[9px] text-slate-400 mt-2 leading-tight">
              Variance calculated via 50 forward passes with stochastic dropout.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-6 border-b border-slate-100 pb-2 flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-indigo-500" /> {chartTitle}
          </h3>
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <XAxis
                      type="number"
                      domain={hasShapValues ? ["dataMin", "dataMax"] : [0, 1]}
                      hide
                    />
                    <YAxis
                      dataKey="name"
                    ...
                    <RechartsTooltip
                      ...
                      formatter={(value: any) => [
                        ...
                        hasShapValues ? "SHAP Value" : "Importance",
                      ]}
                        ...
                        <Cell
                          ...
                          fill={
                            hasShapValues
                              ? entry.shap_value >= 0
                                ? "#ef4444"
                                : "#22c55e" // SHAP: Red for increasing, Green for decreasing
                              : "#6366f1" // Feature importance: Blue
                          }
                        />
                      ))}
                    </Bar>
              ...
              {hasShapValues && (
                <div className="mt-4 flex items-center justify-center gap-6 text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-600">Increases Risk</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-green-500 rounded"></div>
                    <span className="text-slate-600">Decreases Risk</span>
                  </div>
                </div>
              )}
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        ...
                        {hasShapValues ? (
                          ...
                          <span className="text-xs font-mono px-2 py-0.5 rounded bg-indigo-100 text-indigo-700">
                            {(feature.importance * 100).toFixed(1)}%
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-slate-600 mt-1">
                        {feature.clinical_meaning}
                      </p>
                      {hasShapValues && (
                        <p className="text-xs text-slate-500 mt-1">
                          {feature.shap_value >= 0 ? "Increases" : "Decreases"}{" "}
                          cancer risk
                        </p>
                      )}
                    </div>
          ...
          <div className="flex gap-4 mt-4">
            <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold uppercase">
              <div className="w-2.5 h-2.5 bg-indigo-500 rounded-sm" /> High Risk
              Drivers
            </div>
            <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold uppercase">
              <div className="w-2.5 h-2.5 bg-slate-400 rounded-sm" /> Protective
              Factors
            </div>
          </div>
        </div>
        ...
        {(prediction as any).medical_images &&
          (prediction as any).medical_images.length > 0 && (
            <div className="lg:col-span-3 bg-gradient-to-br from-blue-50 to-cyan-50 p-6 rounded-2xl border border-blue-200 shadow-sm">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-6 border-b border-blue-100 pb-2 flex items-center gap-2">
                <FileText className="w-4 h-4 text-blue-500" /> Stored Medical
                Images
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(prediction as any).medical_images.map(
                  (image: any, index: number) => (
                    <div
                      key={index}
                      className="bg-white/70 p-4 rounded-xl border border-blue-100"
                    >
                        ...
                        {image.ai_analysis && image.ai_analysis.prediction && (
                          <div className="pt-2 border-t border-blue-100">
                            ...
                            <div className="text-xs text-slate-600">
                              ...
                              {(
                                image.ai_analysis.prediction.confidence * 100
                              ).toFixed(1)}
                              %)
                            </div>
                          </div>
        ...
        {(prediction as any).image_analysis &&
          (prediction as any).multimodal_fusion && (
            <div className="lg:col-span-3 bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 p-6 rounded-2xl border border-purple-200 shadow-lg">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-center p-3 bg-gradient-to-r from-purple-100 to-pink-100 rounded-lg">
                        ...
                        <span className="text-2xl font-bold text-purple-700">
                          {(
                            prediction as any
                          ).multimodal_fusion.combined_risk.toFixed(1)}
                          %
                        </span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="text-center p-3 bg-blue-50 rounded-lg border border-blue-200">
                          ...
                          <div className="text-lg font-bold text-blue-700">
                            {(
                              prediction as any
                            ).multimodal_fusion.tabular_risk.toFixed(1)}
                            %
                          </div>
                        ...
                        <div className="text-center p-3 bg-rose-50 rounded-lg border border-rose-200">
                          ...
                          <div className="text-lg font-bold text-rose-700">
                            {(
                              prediction as any
                            ).multimodal_fusion.image_risk.toFixed(1)}
                            %
                          </div>
                      ...
                      {(prediction as any).multimodal_fusion.fusion_details && (
                        <div className="p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                          ...
                          <div className="text-sm text-green-600 capitalize">
                            {
                              (prediction as any).multimodal_fusion
                                .fusion_details.agreement_level
                            }{" "}
                            Agreement
                          </div>
                          {(prediction as any).multimodal_fusion.fusion_details
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <div className="text-sm font-semibold text-slate-800">
                            {(prediction as any).image_analysis.model_info
                              ?.name || "ResNet-50"}
                          </div>
                        </div>
                        <div>
                      ...
                      <div className="p-3 bg-slate-50 rounded-lg">
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-semibold text-slate-800">
                            {
                              (prediction as any).image_analysis.prediction
                                .predicted_class
                            }
                          </span>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            ...
                            <div className="p-2 bg-slate-50 rounded">
                              ...
                              <div className="font-semibold text-slate-800">
                                {(
                                  (prediction as any).image_analysis
                                    .tumor_analysis.malignancy_probability * 100
                                ).toFixed(1)}
                                %
                              </div>
                            </div>
                      ...
                      {(prediction as any).image_analysis
                        ...
                        <div className="p-3 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg border border-blue-200">
                          ...
                          <div className="text-sm text-blue-600">
                            ...
                            {
                              (prediction as any).image_analysis
                                .clinical_assessment.urgency_level
                            }
                          </div>
                          <div className="text-xs text-blue-500 mt-1">
                            {
                              (prediction as any).image_analysis
                                .clinical_assessment.time_to_action
                            }
                          </div>
                        </div>
                      )}
                  ...
                  {(prediction as any).image_analysis.recommendations && (
                    <div className="bg-white/80 p-5 rounded-xl border border-purple-100 shadow-sm">
                      ...
                      <div className="space-y-2">
                        {(
                          prediction as any
                        ).image_analysis.recommendations.immediate_actions
                          ?.slice(0, 2)
              ...
              <div className="mt-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-200">
                ...
                <p className="text-xs text-indigo-600 leading-relaxed">
                  {(prediction as any).multimodal_fusion.fusion_method}:
                  Clinical features (patient data, lab results, symptoms) are
                  weighted at 70% while ResNet image analysis contributes 30% to
                  the final risk assessment. This approach leverages both
                  quantitative clinical data and visual pattern recognition for
        ...
        {recommendations && Object.keys(recommendations).length > 0 && (
          <div className="lg:col-span-3 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-6 border-b border-slate-100 pb-2 flex items-center gap-2">
              <FileText className="w-4 h-4 text-emerald-500" /> Evidence-Based
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {recommendations.lifestyle_modifications &&
                recommendations.lifestyle_modifications.length > 0 && (
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2">
                      {recommendations.lifestyle_modifications.map(
              ...
              {recommendations.screening_recommendations &&
                recommendations.screening_recommendations.length > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                    ...
                    <ul className="space-y-2">
                      {recommendations.screening_recommendations.map(
                        (screening: string, index: number) => (
                          <li
                            key={index}
                            className="flex items-start gap-2 text-sm text-slate-700"
                          >
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-[11px] font-bold uppercase text-slate-400 mb-1 tracking-widest">
                ...
                <span className="text-slate-900">
                  {(
                    biasAnalysis?.genderBias ||
                    biasAnalysis?.demographic_bias?.gender_bias ||
                    0
                  ).toFixed(3)}
                </span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-[11px] font-bold uppercase text-slate-400 mb-1 tracking-widest">
                ...
                <span className="text-slate-900">
                  {(
                    biasAnalysis?.ageBias ||
                    biasAnalysis?.demographic_bias?.age_bias ||
                    0
                  ).toFixed(3)}
                </span>
              </div>
            ...
            <div className="p-3 bg-indigo-50/50 rounded-lg border border-indigo-100">
              <p className="text-[10px] text-indigo-700 leading-tight">
                {biasAnalysis?.fairnessWarning ||
                  biasAnalysis?.fairness_metrics?.access_to_care ||
                  "System confirms parity within acceptable clinical thresholds."}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-slate-900 text-white p-6 rounded-2xl shadow-xl relative overflow-hidden">
          ...
          <h3 className="text-sm font-bold uppercase tracking-wider mb-6 border-b border-slate-700 pb-2 flex items-center gap-2 relative z-10">
            <Sliders className="w-4 h-4 text-emerald-400" /> Causal
              ...
              {featureData.length > 0 ? (
                <>
                  <p className="text-slate-400 text-xs leading-relaxed">
                    Adjust <strong>{featureData[0].name}</strong> to explore
                    causal pathways and intervention efficacy. This model uses
                    causal inference to estimate risk delta.
                  </p>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-[11px] font-bold uppercase text-slate-500 tracking-widest">
                      ...
                      <span className="text-emerald-400">
                        Intervention: {simulationValue || featureData[0].value}
                      </span>
                    </div>
                    <input
                      type="range"
                ...
                <p className="text-slate-400 text-xs leading-relaxed">
                  No feature data available for causal simulation.
                </p>
              )}
      ...
      <footer className="mt-12 p-4 bg-amber-50 rounded-xl border border-amber-100 flex items-start gap-3">
        ...
        <p className="text-xs text-amber-800 leading-relaxed">
          <strong>Clinical Disclaimer:</strong> This system is a Research
          Prototype Decision Support System. Predictions are generated via
          ensemble probabilistic models and are for informational purposes only.
          Do not use for definitive diagnosis or treatment planning.
        </p>
      </footer>
    </Layout>
  );
